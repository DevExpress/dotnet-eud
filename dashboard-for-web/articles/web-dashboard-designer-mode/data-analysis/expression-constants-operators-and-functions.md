...
| partitionBy(column1, column2, ...) | Specifies the columns by which the data rows are partitioned. The window function is applied to each partition separately. The _partitionBy_ function can be used only as a w function argument. | partitionBy([Product])|
