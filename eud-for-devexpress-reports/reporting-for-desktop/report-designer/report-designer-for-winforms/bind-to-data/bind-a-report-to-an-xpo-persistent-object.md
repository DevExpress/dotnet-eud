...
Finish the wizard. At this step, the wizard creates an **XPObjectSource** component. This component retrieves all the properties the chosen entity type includes. The [Field List param($match) $path = $match.Groups[1].Value; if ($path -notmatch '^https?://' -and $path -notmatch '^~/' -and $path -notmatch '^\.\./\.\./') { '](' + '../' + $path + '.md)' } else { $match.Value }  reflects the data source structure.
...
You can customize the created **XPObjectSource** component's settings. To do this, select this component in the [Report Explorer param($match) $path = $match.Groups[1].Value; if ($path -notmatch '^https?://' -and $path -notmatch '^~/' -and $path -notmatch '^\.\./\.\./') { '](' + '../' + $path + '.md)' } else { $match.Value }  and switch to the Property grid.
