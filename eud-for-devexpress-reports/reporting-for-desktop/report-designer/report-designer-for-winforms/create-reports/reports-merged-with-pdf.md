...
> [!TIP]
> The [Create a Master-Detail Report (Use Subreports) param($match) $path = $match.Groups[1].Value; if ($path -notmatch '^https?://' -and $path -notmatch '^~/' -and $path -notmatch '^\.\./\.\./') { '](' + '../' + $path + '.md)' } else { $match.Value }  topic describes another way to create a report that merges different documents.
