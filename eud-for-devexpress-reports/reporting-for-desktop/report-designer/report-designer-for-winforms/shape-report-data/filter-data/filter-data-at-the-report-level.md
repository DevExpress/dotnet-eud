...
4. Click the report's smart tag and click the **Filter String** property's ellipsis button.
    ...
    <!-- In the invoked [FilterString Editor param($match) $path = $match.Groups[1].Value; if ($path -notmatch '^https?://' -and $path -notmatch '^~/' -and $path -notmatch '^\.\./\.\./') { '](' + '../' + $path + '.md)' } else { $match.Value } , --> Construct an expression in which the data fields are compared with the required values.
