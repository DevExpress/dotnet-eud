...
| Icon | Command | Description |
|---|---|---|
| ![WPFDesigner_PreviewToolbar_Export](..\/..\/..\/images/img120169.png) | Export | Invokes the [Export Document param($match) $path = $match.Groups[1].Value; if ($path -notmatch '^https?://' -and $path -notmatch '^~/' -and $path -notmatch '^\.\./\.\./') { '](' + '../' + $path + '.md)' } else { $match.Value }  dialog allowing you to export a report document  into one of the supported third-party formats and save the resulting file on a hard drive. |
| ![WPFDesigner_PreviewToolbar_Send](..\/..\/..\/images/img120170.png) | Send | Invokes the [Send via E-Mail param($match) $path = $match.Groups[1].Value; if ($path -notmatch '^https?://' -and $path -notmatch '^~/' -and $path -notmatch '^\.\./\.\./') { '](' + '../' + $path + '.md)' } else { $match.Value }  dialog allowing you to export a report document into one of the supported third-party formats, save the resulting file on a hard drive and attach this file to a new empty message in the default mail program. |
...
| Icon | Command | Description |
|---|---|---|
| ![WPFDesigner_PreviewToolbar_Parameters](..\/..\/..\/images/img120177.png) | Parameters | Shows/hides the [Parameters Panel param($match) $path = $match.Groups[1].Value; if ($path -notmatch '^https?://' -and $path -notmatch '^~/' -and $path -notmatch '^\.\./\.\./') { '](' + '../' + $path + '.md)' } else { $match.Value }  where you can specify report parameters before report preview generation is started. |
| ![WPFDesigner_PreviewToolbar_DocumentMap](..\/..\/..\/images/img120178.png) | Document Map | Shows/hides the [Document Map Panel param($match) $path = $match.Groups[1].Value; if ($path -notmatch '^https?://' -and $path -notmatch '^~/' -and $path -notmatch '^\.\./\.\./') { '](' + '../' + $path + '.md)' } else { $match.Value } , which reflects a report document's structure in a tree-like form. |
...
| ![WPFDesigner_PreviewToolbar_Search](..\/..\/..\/images/img120179.png) | Search | Shows the [Search Panel param($match) $path = $match.Groups[1].Value; if ($path -notmatch '^https?://' -and $path -notmatch '^~/' -and $path -notmatch '^\.\./\.\./') { '](' + '../' + $path + '.md)' } else { $match.Value } , which allows you to find a specified text throughout a report document. |
