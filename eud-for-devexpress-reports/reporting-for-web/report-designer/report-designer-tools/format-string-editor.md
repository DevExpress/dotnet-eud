...
* **Basic Formatting**
    ...
    It is common to format an [Label param($match) $path = $match.Groups[1].Value; if ($path -notmatch '^https?://' -and $path -notmatch '^~/' -and $path -notmatch '^\.\./\.\./') { '](' + '../' + $path + '.md)' } else { $match.Value } 's **Label Text** property. To do this, expand the **Tasks** or **Data** category and click the ellipsis button for the **Text Format String** property.
