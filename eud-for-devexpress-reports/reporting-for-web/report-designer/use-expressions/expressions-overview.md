...
In the Report Designer, properties that support expressions have an `f` button in the [Properties param($match) $path = $match.Groups[1].Value; if ($path -notmatch '^https?://' -and $path -notmatch '^~/' -and $path -notmatch '^\.\./\.\./') { '](' + '../' + $path + '.md)' } else { $match.Value }  panel. Click this button to specify an expression in the invoked Expression Editor.
